...
import requests, pycountry, datetime, descriptions, ast
...
@tool
def fetch_exchange_rates(curency_given: str) -> dict:
...
def fetch_economic_news(input_data: str) -> list:
    ...
    Input should be a dict (or a string repr of one) with country_name and max_articles which is maximum number of articles, can be set to 10.
    """
...
@tool
def generate_pdf_from_html(input_data) -> str:
 
    """Generates a PDF report from given country name, GDP data, exchange rate data, news data and continent data of the country.
    News data is expected to be in HTML format.
    Then the result is saved to a file with the return value as filename.
    Returns: PDF file path which ends with the countries name and _Subsidiary_Report.pdf .
    Input should be a dict with keys country_name, gdp_data, exchange_data, news_data, continent.
    """
    ...
    try:
        ...
        exchange_data= input_data["exchange_data"]
...
def process_news_articles(news_data: any) -> str:
    """Processes raw news articles into a clean HTML format, filtering for relevance to the specified country.
    Input should be a dict with country_name and articles keys.
    Returns HTML string with relevant articles or a 'no news' message.
    """
...
llm = Ollama(model="qwen3:4b", temperature=0.1)
