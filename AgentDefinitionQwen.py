...
# import requests, pycountry, datetime, ast, json, re
...
#     gdp_data: Optional[dict] = Field(default={}, description="GDP data for the country")
#     currency_code: Optional[str] = Field(default="", description="ISO 4217 currency code for the country")
#     exchange_data: Optional[dict] = Field(default={}, description="Exchange rate data for the country")
#     raw_news_articles: Optional[list[dict]] = Field(default = [], description="News articles related to the country")

# class DataBlobOutputParser(BaseOutputParser):

#     def parse(self, text: str) -> DataBlob:
#         try:
#             # 1) Only consider text after </think>
#             if "</think>" in text:
#                 text = text.split("</think>", 1)[1]

#             # 2) Remove a leading "Final Answer:" if present
#             stripped = text.strip()
#             if stripped.startswith("Final Answer:"):
#                 stripped = stripped[len("Final Answer:"):].strip()
#             text = stripped

#             # 3) Extract the first full {...} JSON object
#             #    (?R) allows recursion so nested braces still work
...
#             raise OutputParserException(f"Failed to parse DataBlob: {e}\n\nRaw text:\n{text}")

#     @property
#     def _type(self) -> str:
#         return "data_blob_output_parser"

# class SummaryOutput(BaseModel):
#     summary: str = Field(description="Economic summary of the country")

# llm = ChatOllama(model="qwen3:4b", temperature=0.1)
# #openAI_API_key = "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"
...
# Use empty string, empty dict or empty list for any field that couldn't be collected (e.g. if the corresponding tool was not in the list).
...
#         # unwrap if LangChain returned a dict with "output"
#         if isinstance(response, dict) and "output" in response:
#             raw = response["output"]
#         elif isinstance(response, str):
#             raw = response
        
#         data_blob = DataBlobOutputParser().parse(raw)
#         return data_blob
...
#     except Exception as e:
#         raise ValueError(f"Failed to parse summary output: {e}")
